...
export async function addTimeEntry(project: string, hours: number, date: string) {
  const columnMap = await getColumnMap();
  const rowData = new Array(Math.max(...Object.values(columnMap)) + 1).fill('');
  
  rowData[columnMap.project] = project;
  rowData[columnMap.hoursWorked] = hours.toString();
  rowData[columnMap.dateInvoiced] = date;
  rowData[columnMap.rateType] = 'hourly'; // Default to hourly
...
export async function updateProjectHours(
  ...
  await sheets.spreadsheets.values.batchUpdate({
    ...
    requestBody: {
      ...
      data: [
        {
          range: `Sheet1!${String.fromCharCode(65 + columnMap.hoursWorked)}${rowIndex + 1}`,
